...
for model in "${MODELS[@]}"; do
    for level in "${LEVELS[@]}"; do
        ...
        echo "Running RASTER benchmark for $model ($level)..."
        ./bin/sdf_raster \
            --session "$CONFIG_DIR/raster.json" \
            --screenshot "$CONFIG_DIR/raster_result.png" \
            --output "$CONFIG_DIR/raster_result.json" \
            --headless \
            -u 10 -m 100
    done
done
