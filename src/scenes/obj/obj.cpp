...
bool ObjModel::load (const std::filesystem::path& path) {
    ...
    if (!this->data.vertices.empty ()) {
        ...
        float3 mesh_size = v_max - v_min;
        float3 target_size = float3 (1.998f, 1.998f, 1.998f);
        float3 scale3 = target_size / mesh_size;
        float scale = std::min (scale3.x, std::min (scale3.y, scale3.z));

        float3 min_pos = float3 (-0.999f, -0.999f, -0.999f);

        for (auto& v : this->data.vertices) {
            v.position.x = min_pos.x + scale * (v.position.x - v_min.x);
            v.position.y = min_pos.y + scale * (v.position.y - v_min.y);
            v.position.z = min_pos.z + scale * (v.position.z - v_min.z);
        }
    }
